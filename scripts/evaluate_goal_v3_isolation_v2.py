"""Freeze/verify/run/score isolated Goal v3 v2; no exploration or Policy calls."""
...
from guardian_truth.vnext.goal_v3_isolation_frontend_v2 import PROMPT_SHA256, SCHEMA_SHA256, VERSION
from guardian_truth.vnext.goal_v3_isolation_runner_v2 import CONFIG, StageRunnerV2, immutable
from guardian_truth.vnext.goal_v3_isolation_scoring_v2 import score_case_v2, summarize_v2
from guardian_truth.vnext.goal_v3_isolation_stage_gates_v2 import CORE_GATES, experiment_budget_v2, core_decision_v2, smoke_decision, stress_decision_v2
...
def path(name):
    return OUT / ("goal_v3_isolation_v2_" + name + ".json")
...
def freeze():
    ...
    sources += ["scripts/evaluate_goal_v3_isolation_v2.py", "benchmarks/vnext/goal_v3_isolation_cases_v2.py",
        "docs/vnext/GOAL_V3_ISOLATION_V2_FRONTEND_AND_GATES.md",
        "docs/vnext/GOAL_V3_ISOLATION_V2_BENCHMARK_AND_SCORING.md",
        "docs/vnext/GOAL_V3_ISOLATION_V2_RUN_PROTOCOL.md"]
    sources += sorted(name for name in subprocess.run(["rg", "--files", "tests"], cwd=ROOT,
        check=True, capture_output=True, text=True).stdout.splitlines()
        if "goal_v3" in name and name.endswith("_v2.py"))
    ...
    manifest = {"schema_version": "guardian-goal-v3-isolation-freeze-v2", "architecture_commit": commit,
...
def sealed_stage(stage, frozen):
    ...
    budget = experiment_budget_v2(records, ceiling=frozen["configuration"]["reported_token_ceilings"][stage],
    ...
    runner = StageRunnerV2(OUT, frozen, NoNetwork(), sleeper=lambda _: None)
...
def score(stage):
    ...
    report = {"experiment": "goal_v3_isolation_v2", "stage": stage,
...
def run(stage, env_file):
    ...
    inputs = {row["case_id"]: row["source"] for row in read("inputs")}
    return StageRunnerV2(OUT, frozen, client).run(stage, inputs)

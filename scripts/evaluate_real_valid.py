...
def main() -> None:
    ...
    parser.add_argument("--provider", choices=("bai", "groq", "gemini", "mistral"),
                        default="gemini")
    ...
    parser.add_argument("--reasoning-effort", choices=("none", "low", "medium", "high"),
                        default="none")
    parser.add_argument("--run-label", help="artifact namespace (for example baseline or fix1)")
    ...
    # Resolve provider-local model/key aliases before naming provenance files.
    from guardian_truth.llm_client import ClientConfig
    from guardian_truth.runtime import provider_config
    resolved = provider_config(ClientConfig(), args.provider, model=args.model)
    model = resolved.model
    api_key_env = args.api_key_env or resolved.api_key_env
    adapted = [adapt_competition_input(record) for record in records]
    args.output_dir.mkdir(parents=True, exist_ok=True)
    run_tag = re.sub(r"[^A-Za-z0-9_.-]+", "_",
                     f"{args.provider}__{model}__mot{args.max_output_tokens}"
                     f"__reason-{args.reasoning_effort}")
    artifact_suffix = ("__" + re.sub(r"[^A-Za-z0-9_.-]+", "_", args.run_label)
    ...
    backend = build_live_backend(
        ...
        provider=args.provider, model=model, api_key_env=api_key_env,
    ...
    run_config = {"provider": args.provider, "model": model,
    ...
    by_mode, seals = {}, {}
    for mode in modes:
        progress = args.output_dir / f"{mode}_audit__{run_tag}{artifact_suffix}.json"
    ...
    (args.output_dir / f"prediction_seals{artifact_suffix}.json").write_text(
    ...
    (args.output_dir / f"metrics{artifact_suffix}.json").write_text(

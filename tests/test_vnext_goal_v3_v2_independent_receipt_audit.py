...
AUDIT = runpy.run_path(str(Path(__file__).resolve().parents[1] / "scripts/audit_goal_v3_isolation_v2_receipts.py"))["audit_receipts_v2"]
...
def fixture(tmp_path, outcomes=None):
    ...
    path = tmp_path / "goal_v3_isolation_v2_inputs.json"
    write_new(path, [{"case_id": cid, "source": value} for cid, value in inputs.items()])
    frozen["inputs_sha256"] = file_digest(path)
    write_new(tmp_path / "goal_v3_isolation_v2_inference_freeze.json", frozen)
    run = StageRunnerV2(tmp_path, frozen, FakeClient(outcomes or [completion(), completion()]), sleeper=lambda _: None)
    run.run("S1", inputs)
    return run
...
def test_valid_transport_retry_and_unknown_usage_are_reported_not_guessed(tmp_path):
    fixture(tmp_path, [ChatClientError("timeout", retryable=True), completion()])
    audit = AUDIT(tmp_path, tmp_path, "S1")
    assert audit["transport_retries"] == 1 and audit["physical_requests"] == 2
    assert not audit["usage_complete"] and audit["successful_deliveries"] == 1

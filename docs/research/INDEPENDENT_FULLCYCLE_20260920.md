...
## 2026-09-20 — Этап B2: s2-grounding промежуточно, refusal-канал

- s2_blockrun (пул llama-3.2-11b): F1 0.3636 на 39/46 — строгий grounding-гейт при слабом продюсере РОНЯЕТ recall (гипотеза: продюсер не умеет точно цитировать). Подтверждает необходимость «model-quotes/system-anchors» + сильного продюсера.
- s2_pollinations (gpt-oss-20b): F1 0.5455 на 24/46, precision 0.857 — гейт повышает точность при сильном продюсере; прогон неполный (конфликты рейт-лимитов), резюм запущен.
- Урок инфраструктуры: keyless-провайдеры требуют строгой сериализации заданий (pollinations: очередь=1 на IP; blockrun: free-tier rate; llm7: дневная квота).
- refusal_check v1: маркеры отказа (только текст) → 4 хита, TP1 FP0. Добавлены эскалационные tool-call маркеры (v2).
- Ensemble baseline OR granite = 0.8889 остаётся лучшим; 3 остаточных FN — единый класс premature escalation (см. refusal-канал).

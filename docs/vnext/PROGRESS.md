...
The previous Policy console handle vanished without a report, but the system
process check found no remaining matching job and 69 complete cases/209 captured
request results. Its immutable runner was resumed in a hidden process (PID 13520),
replaying existing receipts without re-querying them; at this update it is live
and beyond case 71/104. No second API job is running. A separate pure post-seal
paired metrics module now calculates strict status/fallback-consistent confusion,
resolution, certificate rates, category errors, exact McNemar and 5000-draw
trajectory-group hierarchical bootstrap. Twelve synthetic tests pass. No blind
labels or X0/vNext end-to-end predictions have been read or generated by it;
actual model gain remains NOT_ESTABLISHED. See PAIRED_METRICS_V1_BOUNDARIES.md.

External source views v6 now split declared-goal provenance from declared-plan
provenance. The latter defaults to UNKNOWN rather than inheriting the goal's
USER/SYSTEM authority. A forged SYSTEM string inside an agent-authored plan
stays text; even a USER-authored list is not automatically mandatory order.
The paired statistics now separates claimed definitive statuses from genuine
independently certificate-valid Core resolution: an invalid/missing certificate
cannot inflate the headline resolution rate. Full checkpoint v26: 1262 passing
tests, no frozen-source integrity errors. Policy remains the only live API stage;
this entry does not claim its final result or Goal v3 implementation.

A read-only R22 delivery inventory gate now checks every required JSON/JSONL/doc
listed in the frozen requirements matrix without opening private blind labels or
writing missing artifacts. Its current audit finds 9 present, 17 missing, zero
invalid; artifact inventory is INCOMPLETE and is explicitly not semantic,
certificate, document-agreement or promotion proof. Four synthetic gate tests
...
## E2E-agent-1 causal repair (dev cycle, frozen at ff40cbd)

- Diagnosed the confirmed defect: claim-channel uncertainty (CLAIM_UNTYPED/ENTITY_UNBOUND/SOURCE_UNBOUND/TIME_UNBOUND) entered hard_reasons -> every world's markers, and the baseline solver short-circuits markered worlds to UNKNOWN, masking certified independent Policy/Goal/factual violations (11 dev cases, all gold=PROVED_ERROR).
- Implemented the limited causal repair, flag-gated (repair=None = byte-identical A0): A1 scoped claim-UNKNOWN composition (scoped_status_v1: FALSE and UNKNOWN = FALSE; certifiable conjuncts require axis enumeration-completeness; PROVED_NO_ERROR keeps full baseline strictness); A2 deterministic claim value anchoring (claim_adapter_v2: LITERAL_VALUE + COMPLETION_EFFECT, same pure function in pipeline and checker); A3 catalog-identity binding of unexercised action/event atoms (binding_repair_v1, no argument checks/entity path, ledger-only call proofs). Baseline solve(), frontends, prompts, schemas, program space untouched. Disclosed crash fix in policy_composition_v1 (synth-UNAVAILABLE TypeError path).
- Controlled tests: 25 new tests incl. all 9 mandatory scenarios (tests/test_e2e_scoped_repair.py); 45/45 prior e2e tests green; full suite shows only the 10 pre-existing failures (identical to clean HEAD).
- Causal arms over the sealed E2E V1 semantic outputs (zero LLM, deterministic replay; A0 hard-verified == sealed E0 69/69): A0 F1 0.308 -> A1/A2/A3 0.680 (TP 6->17, FP 1 unchanged, 11 corrections / 0 regressions, McNemar p=0.001, Newcombe [+7.3,+26.3]pp). false_certified_NO_ERROR = 0 everywhere; all definitives certified; gold-UNRESOLVED->system-ERROR list empty.
- A2: 2 anchors fired, 0 outcome changes (target cases lack the paired FAILURE result event - corpus construction gap). A3: 27 legitimate repairs, 0 outcome changes (decisive unbound atoms are absent from the case tool catalogs; binding them would invent tools - refused on soundness).
- Frozen selection per preregistered rule: A1 (minimal repair; ties broken to the lowest arm). Frozen commit ff40cbd897eaadafd2a8bb89578cad2cff49bd27; manifest outputs/vnext/e2e_agent1_repair_freeze.json.
- STOP before headline fresh evaluation: no shared Agent-1/Agent-2 holdout is provided in this repository for this cycle (agent-2's own holdout artifacts are not marked shared); future shared gold neither viewed nor generated. Residual 15 gold-ERROR UNRESOLVED cases classified (binding transport failures, corpus catalog gaps incl. missing FAILURE results and missing actor atoms, GUARD-frame goal semantics, state-observation abstention) - see docs/vnext/e2e/E2E_AGENT1_REPAIR_RESULTS_RU.md.

...
def write_case_audit(adapted_cases: list[CompetitionInput], by_mode: dict[str, list[dict]],
    ...
    with path.open("w", encoding="utf-8") as stream:
        for adapted in adapted_cases:
            ...
            premises = [{"fact": f"declared_argument_schema:{item['name']}",
                         "origin": "EXPLICIT_SCHEMA", "trusted": True,
                         "source_start": item["source"]["start"],
                         "source_end": item["source"]["end"],
...
def write_premise_coverage(adapted_cases: list[CompetitionInput], path: Path) -> None:
    with path.open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=["id", "tool", "premise", "origin",
                                                    "trusted", "source_start", "source_end"])
        writer.writeheader()
        for adapted in adapted_cases:
            for item in adapted.tool_declarations:
                writer.writerow({"id": adapted.case.case_id, "tool": item["name"],
                                 "premise": "argument_schema", "origin": "EXPLICIT_SCHEMA",
                                 "trusted": True, "source_start": item["source"]["start"],
                                 "source_end": item["source"]["end"]})

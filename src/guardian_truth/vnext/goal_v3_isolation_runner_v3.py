...
from .goal_v3_isolation_frontend_v3 import grounding_payload_v2, prompt_messages_v3, proposal_issues_v3
from .goal_v3_isolation_stage_gates_v3 import experiment_budget_v3
...
CONFIG = {"provider": "groq", "model": "qwen/qwen3.8-27b",
    ...
    "temperature": 0, "max_output_tokens": 4096, "timeout_seconds": 180,
    ...
    "retry_same_payload_only": True, "retry_backoff_seconds": 60,
    ...
    "raw_completion_storage": "HASH_VALID_PROPOSAL_AND_VALUE_FREE_SCHEMA_PATHS",
    "failed_usage_rule": "CONTINUE_KNOWN_TRANSPORT_FAILURE_WITH_USAGE_UNKNOWN",
    "success_missing_usage_rule": "STOP", "abandoned_capture_rule": "STOP_NO_RESEND",
...
def repair_v3(text):
    if not isinstance(text, str):
        return None, False, False, "NO_TEXT", [{"code": "JSON_INVALID", "path": "", "expected_type": None, "actual_type": None}]
    ...
    if not raw:
        ...
        for prefix, name in (("```json\n", "EXACT_JSON_FENCE"), ("```\n", "EXACT_FENCE")):
            if stripped.startswith(prefix) and stripped.endswith("```"):
                ...
                code = name
                break
    issues = list(proposal_issues_v3(value)) if valid and isinstance(value, dict) else [{"code": "JSON_INVALID", "path": "", "expected_type": None, "actual_type": None}]
...
class StageRunnerV3:
    ...
    def path(self, name):
        return self.output / ("goal_v3_isolation_v3_" + name + ".json")
    ...
    def physical(self, case_id, ordinal, source, messages):
        ...
        telemetry = {"transport_status": "ERROR", "error_category": None, "retryable": False,
            ...
            "served_model": None, "latency_ms": 0, "repair_code": "NO_CAPTURE", "schema_issue_codes": [], "schema_issues": []}
        proposal = None
        if request_path.exists():
            ...
            try:
                ...
                proposal, raw, repaired, code, issues = repair_v3(completion.content)
                telemetry.update(transport_status="SUCCESS", raw_schema_valid=raw,
                    postrepair_schema_valid=repaired, repair_code=code, schema_issue_codes=[issue['code'] for issue in issues], schema_issues=issues,
    ...
    def case(self, case_id, source):
        messages, records = prompt_messages_v3(source), []
    ...
    def run(self, stage, inputs):
        ...
        try:
            ...
            for case_id in ids:
                ...
                budget = experiment_budget_v3(physical, ceiling=self.config["reported_token_ceilings"][stage],

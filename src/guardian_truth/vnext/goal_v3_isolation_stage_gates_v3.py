"""v3 gates: same semantic thresholds, separately declared failed-usage rule.

No frozen v1/v2 gate, score or artifact is modified.
"""
...
def budget_after_case(physical_records: Sequence[dict], *,
    ...
    advertised output limit, as observed in v1. Successful/abandoned missing
    usage is fail-closed. Known transport failures may have unknown usage;
    their physical receipts remain present and accounting stays incomplete.
    """
    ...
    for record in physical_records:
        ...
        if type(tokens) is not int or tokens < 0:
            # Known failed transport may omit usage; do not invent zero billing.
            # A missing successful or abandoned capture still stops admission.
            if (record.get("transport_status") == "ERROR"
                    and record.get("error_category") in {"timeout", "rate_limit", "server", "connection"}):
                continue
...
def experiment_budget_v3(records, *, ceiling, enforce_token_ceiling):
    """v3: known transport failures may omit usage; successful/abandoned may not."""
...
def smoke_decision(rows: Sequence[dict], physical_records: Sequence[dict], *,
    ...
    budget = experiment_budget_v3(physical_records, ceiling=S1_REPORTED_TOKEN_CEILING,

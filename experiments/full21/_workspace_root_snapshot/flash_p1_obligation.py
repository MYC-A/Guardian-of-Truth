...
def run_one(provider: str, case: dict, susp: dict, digest: str | None) -> dict:
    ...
    for attempt in range(3):
        msgs = list(obli_msgs)
        if attempt == 2:
            msgs.append({"role": "user", "content": "Your previous reply was not valid JSON. Reply with ONLY the JSON object."})
    ...
    chk = None
    for attempt in range(3):
        msgs = list(check_msgs)
        if attempt == 2:
            msgs.append({"role": "user", "content": "Your previous reply was not valid JSON. Reply with ONLY the JSON object."})
        raw2 = complete(provider, msgs, temperature=0, max_tokens=700)
        try:
...
def main() -> int:
    ...
    ap.add_argument("--provider", default="blockrun",
                    help="primary provider; secondary is the other channel")
    ap.add_argument("--shard", default="0/1",
                    help="process only keys where hash(key)%%M==S (format S/M)")
    args = ap.parse_args()
    ...
    try:
        sh_s, sh_m = (int(x) for x in args.shard.split("/"))
    except Exception:
        sh_s, sh_m = 0, 1
    pending = [(k, s) for k, s in todo
               if (k not in done or done[k].get("status") == "FAILED")
               and (sh_m <= 1 or hash(k) % sh_m == sh_s)]
    ...
    for i, (key, susp) in enumerate(pending, 1):
        ...
        if case is None:
            ...
            ok = False
            chain = [args.provider] + [x for x in ("blockrun", "pollinations") if x != args.provider]
